...
<img src='images/goodhealth.png' style='position:absolute;left:3%;top:0;width:90%;'>
...
<textarea name='newpubtext' placeholder='    
     ...
     Share your challenge NOW !' style='border-radius:100px;outline:none;height:80px;border:1px solid #279b48;background-color:white;width:70%;'></textarea>
<br>
...
if(isset($_POST['ajouter'])){
	...
	mysqli_query($con,"INSERT INTO pubs VALUES ('','goodhealth','$fullname','$email','$newpubtext','$path','novideo',NOW(),NOW())");
	}else{
	$infophoto = pathinfo($_FILES['photo']['name']);
	$extension = $infophoto['extension'];
	$newimagename = $code_aleatoire;
	...
	$path = "photos/".$newimagename.".".$extension;
	move_uploaded_file($_FILES['photo']['tmp_name'],$path);
	mysqli_query($con,"INSERT INTO pubs VALUES ('','goodhealth','$fullname','$email','notext','$path','novideo',NOW(),NOW())");
	}
	}
	if($video != ''){
	if($newpubtext != ''){
	...
	mysqli_query($con,"INSERT INTO pubs VALUES ('','goodhealth','$fullname','$email','$newpubtext','nophoto','$path',NOW(),NOW())");
	}else{
	$infovideo = pathinfo($_FILES['video']['name']);
	$extension = $infovideo['extension'];
	$newvideoname = $code_aleatoire;
	$path = "videos/".$newvideoname.".".$extension;
	move_uploaded_file($_FILES['video']['tmp_name'],$path);
	mysqli_query($con,"INSERT INTO pubs VALUES ('','goodhealth','$fullname','$email','notext','nophoto','$path',NOW(),NOW())");
	}



	}
	header("Location:goodhealth.php");
...
<br><br>
<?php
$querypubs = mysqli_query($con,"SELECT * FROM pubs WHERE cat='goodhealth' ORDER BY -id");
while($rowspubs = mysqli_fetch_array($querypubs)){
?>
<div style='width:45%;background-color:white;border:2px solid #279b48;height:250px;float:left;margin:1%;padding:2% 1%;border-radius:10px;position:relative;'>
